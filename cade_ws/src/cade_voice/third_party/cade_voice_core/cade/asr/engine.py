...
class ASREngine:
    ...
    def __init__(
        ...
        streaming_pre_roll_sec: float = 0.5,
        streaming_post_roll_sec: float = 0.5,
    ):
        """
        ...
        Args:
            ...
            streaming_pre_roll_sec: Audio to replay after ASR gate reopens.
            streaming_post_roll_sec: Audio to keep decoding after endpoint detection.
        """
        self.model_dir = Path(model_dir)
        self.sample_rate = sample_rate
        self._text_replacements = text_replacements
        self._needs_text_norm = model_type == "streaming_nemotron"
        self._streaming_pre_roll_frames = max(
            0, int(float(streaming_pre_roll_sec) * sample_rate)
        )
        self._streaming_post_roll_frames = max(
            0, int(float(streaming_post_roll_sec) * sample_rate)
        )
        self._metrics = {
            "decode_count": 0,
            "decode_total_s": 0.0,
        ...
        logger.info(
            f"ASR engine initialized: {self.active_model_name}, provider={provider}, "
            f"streaming={self._is_streaming}, "
            f"pre_roll={self._streaming_pre_roll_frames / sample_rate:.2f}s, "
            f"post_roll={self._streaming_post_roll_frames / sample_rate:.2f}s"
        )
    ...
    def _handle_streaming_endpoint(
        self,
        stream,
        ...
        if text:
            ...
            if (
                self._metrics["consecutive_empty_finals"]
                > self._metrics["max_consecutive_empty"]
            ):
                self._metrics["max_consecutive_empty"] = self._metrics[
                    "consecutive_empty_finals"
                ]
            if self._metrics["consecutive_empty_finals"] >= 5:
                logger.warning(
                    f"Consecutive empty finals: {self._metrics['consecutive_empty_finals']}"
                )
            ...
            try:
                while True:
                    ...
                    if not gate_open:
                        self._recognizer.reset(stream)
                        if len(pre_roll) > 0:
                            stream.accept_waveform(self.sample_rate, pre_roll)
                            pre_roll = np.array([], dtype=np.float32)
                        gate_open = True

                    stream.accept_waveform(self.sample_rate, samples)
                    if self._streaming_pre_roll_frames > 0:
                        pre_roll = np.concatenate([pre_roll, samples])
                        pre_roll = self._trim_audio(
                            pre_roll, self._streaming_pre_roll_frames
                        )
                    ...
                    if self._recognizer.is_endpoint(stream) and not endpoint_pending:
                        endpoint_pending = True
                        post_roll_remaining = self._streaming_post_roll_frames

                    if endpoint_pending:
                        post_roll_remaining -= len(samples)
                        if post_roll_remaining <= 0:
                            self._handle_streaming_endpoint(stream, callback)
                            if save_dir:
                                segment_count += 1
    ...
    def _start_listening_pulse_streaming(
        ...
        try:
            while True:
                ...
                if self._streaming_pre_roll_frames > 0:
                    pre_roll = np.concatenate([pre_roll, samples])
                    pre_roll = self._trim_audio(pre_roll, self._streaming_pre_roll_frames)
    ...
    @staticmethod
    def _drain_queue(audio_queue: queue.Queue) -> List[np.ndarray]:
        drained = []
        while True:
            try:
                drained.append(audio_queue.get_nowait())
            except queue.Empty:
                return drained

    @staticmethod
    def _trim_audio(audio: np.ndarray, max_frames: int) -> np.ndarray:
        if max_frames <= 0 or len(audio) <= max_frames:
            return audio
