...
class VisionSkill(BaseSkill):
    ...
    def find_person(self, room: Optional[str] = None,
        ...
        Args:
            ...
            cloth_color: 衣服颜色（可选）
            category: 自定义类别文本（可选，如 "person wearing red clothes"）
            timeout: 超时时间
    ...
    def get_nearest_person_attributes(self, timeout: float = 5.0) -> Dict[str, Any]:
        """
        返回离机器人最近的人的视觉属性。

        Returns:
            dict 或 None（作为 result 字段返回）:
            {"cloth_color": "blue", "cloth_type": "t-shirt", ...}
        """
        return self.execute("get_nearest_person", timeout=timeout)

    def filter_by_attributes(self, attributes: dict, timeout: float = 5.0) -> Dict[str, Any]:
        """
        从当前帧找出匹配所有指定属性的人。
        ...
        Returns:
            {"status": "SUCCESS", "result": {"persons": [...], "count": N}}
        """
        return self.execute("filter_by_attributes", timeout=timeout, **attributes)
