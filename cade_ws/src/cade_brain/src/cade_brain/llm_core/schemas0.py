...
class BindPersonAction(BaseModel):
    """绑定人名到视觉外观"""
    type: Literal["bind_person"] = "bind_person"
...
class RecallAction(BaseModel):
    ...
    what: str = Field(default="appearance", description="'appearance' 或 'info'")
    name: str = Field(..., description="人物名称")
...
class RememberAction(BaseModel):
    ...
    name: str = Field(..., description="人物名称")
    key: str = Field(default="", description="属性名，如 'drink'")
    value: str = Field(default="", description="属性值，如 'orange juice'")


class FindPersonByAttributesAction(BaseModel):
    """按外观属性找人"""
    type: Literal["find_person_by_attributes"] = "find_person_by_attributes"
    attributes: dict = Field(default={}, description="外观属性，如 {cloth_color: 'blue'}")


class SpeakAction(BaseModel):
    """TTS 播放文本"""
    type: Literal["speak"] = "speak"
    text: str = Field(default="", description="要说的话")
...
def parse_action(action_dict: dict) -> RobotAction:
    ...
    action_map = {
        ...
        "bind_person": BindPersonAction,
        "recall": RecallAction,
        "remember": RememberAction,
        "find_person_by_attributes": FindPersonByAttributesAction,
        "speak": SpeakAction,
    }
