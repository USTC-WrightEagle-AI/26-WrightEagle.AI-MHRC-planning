    ...
    def _execute_action(self, action: RobotAction) -> bool:
        ...
        try:
            ...
            if action_type == "navigation":
                ...
                if result and result.get("status") == "SUCCESS" and position:
                    self.current_position = position

            # ==================== 视觉相关原子动作 (映射到 VisionSkill) ====================
            elif action_type == "gesture_recognition":
                result = self.vision_skill.find_person(
                    room=getattr(action, 'room', None),
                    gesture=getattr(action, 'gesture', None)
                )

            elif action_type == "clothes_recognition":
                result = self.vision_skill.find_person(
                    room=getattr(action, 'room', None),
                    cloth_color=getattr(action, 'cloth_color', None)
                )

            elif action_type == "object_search":
                result = self.vision_skill.find_object(
                    object_name=getattr(action, 'object_name', None),
                    room=getattr(action, 'room', None)
                )

            elif action_type == "gesture_counting":
                result = self.vision_skill.count_people(
                    room=getattr(action, 'room', None),
                    gesture=getattr(action, 'gesture', None)
                )

            elif action_type == "clothes_counting":
                # map clothes_counting to count_people with category filter
                cloth = getattr(action, 'cloth_color', None)
                category = f"person wearing {cloth} clothes" if cloth else None
                result = self.vision_skill.count_people(
                    room=getattr(action, 'room', None),
                    category=category
                )

            elif action_type == "person_tracking":
                # If a person_id is provided, try following via NavSkill; otherwise, fall back to vision find
                if getattr(action, 'person_id', None):
                    result = self.nav_skill.follow_person(person_name=getattr(action, 'person_id'))
                    if result and result.get("status") == "SUCCESS" and getattr(action, 'person_pos', None):
                        self.current_position = action.person_pos
                else:
                    result = self.vision_skill.find_person(room=getattr(action, 'person_pos', None))

            # ==================== 名字绑定 / 记忆 相关 ====================
            elif action_type == "name_recognition":
                # get nearest person's attributes and optionally bind to name
                attrs = self.vision_skill.get_nearest_person_attributes()
                if attrs:
                    if getattr(action, 'bind_to_appearance', True):
                        self.episodic.bind_person(action.name, attrs)
                        result = {"status": "SUCCESS",
                                  "result": f"Bound {action.name} to appearance: {attrs}"}
                    else:
                        result = {"status": "SUCCESS", "result": attrs}
                else:
                    result = {"status": "FAILED", "error": "No person detected nearby"}

            # ==================== 对话 / 简单动作 ====================
            elif action_type == "talking":
                result = {"status": "SUCCESS", "result": getattr(action, 'text', '')}

            # ==================== 抓取/放置/其他物品动作 (委托给 NavSkill 或泛化执行) ====================
            elif action_type == "object_grasp":
                result = self.nav_skill.pick_and_bring(
                    object_name=getattr(action, 'object_name', None),
                    placement=getattr(action, 'object_position', None)
                )

            elif action_type == "object_dump":
                # try to use a NavSkill helper if available, otherwise generic execute
                if hasattr(self.nav_skill, 'place_object'):
                    result = self.nav_skill.place_object(target_position=getattr(action, 'target_position', None))
                else:
                    result = self.nav_skill.execute(action_type, timeout=60.0)

            # ==================== 兜底：尝试交给 NavSkill 执行通用命令 ====================
            else:
                result = self.nav_skill.execute(action_type, timeout=60.0)
