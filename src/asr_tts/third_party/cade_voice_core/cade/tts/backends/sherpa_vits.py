"""sherpa-onnx VITS/Piper TTS backend."""
...
class SherpaVitsBackend:
    """VITS/Piper backend backed by ``sherpa_onnx.OfflineTts``."""
    ...
    def _create_tts(self):
        ...
        if not model_file or not tokens_file:
            raise FileNotFoundError(f"VITS/Piper model files not found in {self.model_dir}")

        lexicon = self._find_file("lexicon.txt", "*lexicon*.txt")
        ...
        if not tts_config.validate():
            raise ValueError(f"Invalid VITS/Piper TTS configuration for {self.model_dir}")
